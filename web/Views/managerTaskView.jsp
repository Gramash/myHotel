...
<table id="applications" style=" background-color: mintcream ">
    ...
    <c:forEach items="${applList}" var="appl">
        <form method="POST" action="/controller">
            <input type="hidden" name="command" value="filterSuggestions">
            <tr>
...
<table id="products" style=" background-color: mintcream ">
    <tr>
        <th>image</th>
        <th>Room #</th>
        <th>Sleeps</th>
            ...
            <td align="center">
                ...
                <c:if test="${product.taken}">
                    <form action="/controller">
                        <input type="hidden" name="command" value="managerTask">
                        <input type="date" name="checkInProd" value="${checkIn}"/> <br>
                        <input type="date" name="checkOutProd" value="${checkOut}"/> <br>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <form action="/controller">
                                ...
                                <input type="hidden" name="roomNo" value="${product.roomNo}">
...
<table id="suggestions list" style="background-color: mintcream">
    ...
    <c:forEach items="${suggestionsList}" var="suggestion" varStatus="loop">
        <tr>
            <form action="/controller">
                <input type="hidden" name="command" value="submitSuggestion"/>
                <input type="hidden" name="appId" value="${appId}"/>
                <input type="hidden" name="userId" value="${userId}"/>
                <td>
                    <img src="${suggestion.image}" height="125" width="150">
                </td>
                <td>
                    <input type="hidden" name="roomNo" value="${suggestion.roomNo}"/>
                        ${suggestion.roomNo}
                </td>
                <td>
                    <input type="hidden" name="roomNo" value="${suggestion.sleeps}"/>
                        ${suggestion.sleeps}
                </td>
                <td>
                    <input type="hidden" name="roomNo" value="${suggestion.clazz}"/>
                        ${suggestion.clazz}
                </td>
                <td>
                    <input type="hidden" name="roomNo" value="${suggestion.price}"/>
                        ${suggestion.price}
                </td>
                <td>
